...
%%%
%%% Created : 03. сен 2014 12:01
%%%-------------------------------------------------------------------
-module(api_find_all_history).
    ...
    try
      FindAll =
        case AllBindings = reading_the_req_body(Req) of
          false -> emongo:find(model, "history");
          _ ->
            emongo:find(model, "history", [{"created_at", [{'>=', proplists:get_value(<<"date_from">>, AllBindings)}, {'=<', proplists:get_value(<<"date_to">>, AllBindings)}]}])
        end,
      {200, FindAll}
    catch
